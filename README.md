...
Remember that the style is of vector of shape `(B, encoder_hidden_dim)` while the
decoder generates curves of shape `(B, seq_dim, 12)`.\
During the generation process,
the decoder starts with a unique zero point (`(B, 1, 12)` filled with 0)
and the style vector.
