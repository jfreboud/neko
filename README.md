...
Currently, `neko` supports Python 3.9 and above (but I did not check recent versions).
...
paragraph without having to actually [train the models](#train-an-encoder-and-a-decoder)
first.
...
`--device`: the device used to train the models (I tested mps and cuda). \
...
`--device`: the device used to run the models (I tested mps and cuda). \
...
<figure float="left">
  ...
  <figcaption>Generated curves: same lead but different patient</figcaption>

  <img src="data/in/pat4_lead1_truth.png" width="150" />
  <img src="data/in/pat3_lead1_truth.png" width="150" />
  <figcaption>Corresponding ground truth</figcaption>
</figure>
...
<figure float="left">
  ...
  <figcaption>Generated curves: same patient but different lead</figcaption>
