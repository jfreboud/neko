...
for training classifiers of cardiovascular risk. The clinical data we
talked in the [previous paragraph](#unsupervised-learning) may be used as well.

Still, we should challenge any clinical data that is
fed to the classifier in order to make its predictions.
We already want to rely on the ECGs with tailored `features` extraction,
other clinical data may add noise to the global signal. Features importance
